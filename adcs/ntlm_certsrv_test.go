...
// newNtlmCertsrv builds an NtlmCertsrv struct directly, bypassing the
// NewNtlmCertsrv constructor's live verifyNtlm network check, and points its
// httpClient at the given test server client.
func newNtlmCertsrv(url string, client *http.Client) *NtlmCertsrv {
	return &NtlmCertsrv{
		url:        url,
		username:   "testuser",
		password:   "testpass",
...
// dispositionHTMLBody builds a body that matches the "Disposition message:"
// and optionally "LastStatus:" regexes used by GetExistingCertificate.
func dispositionHTMLBody(dispositionSuffix string, includeLastStatus bool) string {
	body := "Certificate Request Processing\r\nDisposition message:reason\t\t" + dispositionSuffix + "\r\n"
	if includeLastStatus {
		body += "LastStatus:info\t\tSome Last Status\r\n"
	}
	return body
}

func TestNtlmRequestCertificate(t *testing.T) {
	t.Run("ready via direct pkix content type", func(t *testing.T) {
		...
		s := newNtlmCertsrv(server.URL, server.Client())
	...
	t.Run("no id match, fallback unknown error", func(t *testing.T) {
		...
		_, _, _, err := s.RequestCertificate("csr", "template")
		assert.Error(t, err)
		assert.EqualError(t, err, "Unknown error occurred")
	})
}

func TestNtlmGetExistingCertificate(t *testing.T) {
...
func TestNtlmObtainCaCertificate(t *testing.T) {
	...
	t.Run("non-200 status panics on nil error dereference", func(t *testing.T) {
		...
		s := newNtlmCertsrv(server.URL, server.Client())
		_, err := s.GetCaCertificate()
		assert.EqualError(t, err, "ADCS Certsrv response status 500 Internal Server Error")
	})
}

func TestNtlmVerifyNtlm(t *testing.T) {
...
func TestNewNtlmCertsrv(t *testing.T) {
	t.Run("verify false with credentials set", func(t *testing.T) {
		c, err := NewNtlmCertsrv("http://example.invalid", "user", "pass", nil, false)
		assert.NoError(t, err)
		assert.NotNil(t, c)
	})

	t.Run("verify false with credentials empty", func(t *testing.T) {
		c, err := NewNtlmCertsrv("http://example.invalid", "", "", nil, false)
		assert.NoError(t, err)
		assert.NotNil(t, c)
	})

	t.Run("verify true against live test server", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusOK)
		}))
		defer server.Close()

		c, err := NewNtlmCertsrv(server.URL, "", "", nil, true)
		assert.NoError(t, err)
		assert.NotNil(t, c)
	})
}
