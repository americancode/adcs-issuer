...
For production configurations, the CA can instead be stored in a Secret or ConfigMap and referenced with `spec.caBundleRef`. The reference requires `name`; `kind` defaults to `Secret` and `key` defaults to `ca.crt`. For separate root and issuing certificate entries, use `keys` instead of `key`; values are concatenated in order. The existing inline `spec.caBundle` field remains supported.
